...
export const getProductMiniType = async (miniType, page, limit) => {
  if (miniType) {
    const res = await axios.get(
      `${process.env.REACT_APP_API_URL}/product/get-all?filter=miniType&filter=${miniType}&limit=${limit}&page=${page}`
    );
    return res.data;
  }
};
export const getProductBrand = async (brand, page, limit) => {
  if (brand) {
    const res = await axios.get(
      `${process.env.REACT_APP_API_URL}/product/get-all?filter=brand&filter=${brand}&limit=${limit}&page=${page}`
    );
    return res.data;
  }
};
...
export const getAllMiniTypeProduct = async (type) => {
  const res = await axios.get(
    `${process.env.REACT_APP_API_URL}/product/get-all-mini-type/${type}`
  );
  return res.data;
};
